...
var Elephant = function () {
  ...
  _createClass(Elephant, [{
    ...
    key: 'convertDashToCamelCase',
    value: function convertDashToCamelCase(name) {
      var camelName = name.replace(/-([a-z])/g, function (match, substring) {
    ...
    value: function writeScssFile(glyphs, config) {
      ...
      glyphs.forEach(function (glyph) {
        ...
        if (config.dashToCamelCase) {
          glyphName = _this.convertDashToCamelCase(glyph.name);
        }

        output += '$' + config.variablePrefix + glyphName + ': \'\\' + glyph.hex + '\';\n';
      });
